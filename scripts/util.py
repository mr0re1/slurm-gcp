...
def reservation_placement(reservation):
    ...
    if url:
        return url.split('/')[-1] # keep only the last component - name
    return None
...
class Lookup:
    ...
    def reservation(self, name):
        ...
        """
        resp = ensure_execute(self.compute.reservations().aggregatedList(
            project=self.project,  filter=f"name={name}"))
        
        reservation = None
        for _, e in resp['items'].items():
            for r in e.get('reservations', []):
                assert reservation is None, f"multiple reservations '{name}' found in '{self.project}'."
                reservation = r
        
        assert reservation is not None, f"reservation '{name}' not found in '{self.project}'."
        return reservation
